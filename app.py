...
def generate_image_from_prompt(full_prompt, quality_key="✨ Ultra 標準（gpt-image-2 medium）", size_key="⬜ 正方形 1024×1024（預設）"):
    ...
    img_quality = mode.get("quality")  # high / medium / low / None
    errors = []
    ...
    for model in mode["generate_models"]:
        try:
            ...
            st.caption(f"✓ 使用 **{model}** 生成（{model_size}{q_label}）")
...
def generate_image_from_reference(reference_bytes, full_prompt, quality_key="✨ Ultra 標準（gpt-image-2 medium）", size_key="⬜ 正方形 1024×1024（預設）"):
    ...
    for model in edit_models:
        try:
            ...
            q_label = f"｜{img_quality}" if img_quality and model.startswith("gpt-image") else ""
            st.caption(f"✓ 使用 **{model}** 編輯（{model_size}{q_label}）")
            return _decode_result(result)
        except Exception as e:
            errors.append(f"{model}: {e}")
            continue
