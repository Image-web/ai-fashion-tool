...
def show_generate(email, usage, daily_limit):
    ...
    with st.container():
        ...
        with col1:
            ...
            reference_file = st.file_uploader("參考圖片（選填）",
        ...
        with col2:
            ...
            generate_btn = st.button("✦ 全新生成", use_container_width=True, key="gen_new",
                type="primary")
                ...
                for i in range(num_images):
                    ...
                    with st.spinner(f"AI 生成中：{label}，請稍候..."):
                        try:
                            ...
                            upload_to_supabase(img_bytes)
                            save_design(email, full_prompt, "", upload_to_supabase(img_bytes))
                        except Exception as e:
                            st.error(f"第 {i+1} 張生成失敗：{e}")

                if results:
                    st.success(f"✓ 成功生成 {len(results)} 張")
                    if len(results) == 1:
                        st.session_state["current_img"] = results[0]
                        st.session_state["edit_history"] = [{
                            "bytes": results[0], "prompt": full_prompt,
                            "label": f"v1 原始生成"
                        }]
                        st.rerun()
                    else:
                        ...
                        for idx, ib in enumerate(results):
                            with gcols[idx % 3]:
                                ...
                                st.download_button(f"⬇ 下載", data=ib,
                                    file_name=f"fashion_{idx+1}_{uuid.uuid4().hex[:6]}.png",
                                    mime="image/png", key=f"dl_new_{idx}")
                                if st.button(f"✏️ 選此張繼續修改", key=f"sel_{idx}",
                                             use_container_width=True):
                                    st.session_state["current_img"] = ib
                                    st.session_state["edit_history"] = [{
                                        "bytes": ib, "prompt": full_prompt,
                                        "label": f"v1 原始生成（第{idx+1}張）"
                                    }]
                                    st.rerun()
    ...
    if has_current:
        ...
        with edit_col:
            ...
            eq_key   = st.selectbox("畫質／模型", list(QUALITY_MODES.keys()), key="qk_edit")
            er_key   = st.selectbox("長寬比", list(ASPECT_RATIOS.keys()), key="rk_edit")
            erez_key = st.selectbox("解析度", list(RESOLUTION_TIERS.keys()), index=1, key="rek_edit")
