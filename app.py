...
def show_generate(email, usage, daily_limit):
    ...
    if "current_label" not in st.session_state: st.session_state["current_label"] = ""
    ...
    # ════════════════════════════════════════════
    if has_current:
        tab_new, tab_edit = st.tabs(["✦ 全新生成", "✏️ 繼續修改目前圖片"])
    else:
        tab_new = st.container()
        tab_edit = None
    ...
    with tab_new:
        ...
        with col1:
            ...
            reference_file = st.file_uploader("參考圖片（選填，用於圖片修改）",
                type=["png","jpg","jpeg"], key="ref_new")
            if reference_file:
        ...
        with col2:
            ...
            num_images = st.selectbox("生成數量", [1,2,3,4,5,6,8,10], index=0,
                help="每張獨立佔用每日額度", key="num_new")
            if num_images > remaining:
                st.warning(f"今日剩餘 {remaining} 張，請調整數量")

            # 若有目前圖，提示可繼續修改
            if has_current:
                st.info("💡 點「繼續修改目前圖片」可對現有圖片下指令修改")

            generate_btn = st.button("✦ 開始生成", use_container_width=True, key="gen_new")
            ...
            if generate_btn:
                if not user_prompt.strip():
                    ...
                    if results:
                        ...
                        # 單張：進入可繼續編輯模式
                        if len(results) == 1:
                            st.session_state["current_img"]   = results[0]
                            st.session_state["current_label"] = user_prompt[:40]
                            st.session_state["edit_history"]  = [{
                                "bytes":  results[0],
                                "prompt": full_prompt,
                                "label":  f"原始生成 — {user_prompt[:30]}"
                            }]
                            st.image(results[0], caption="生成結果（可在「繼續修改」頁繼續編輯）", use_container_width=True)
                            st.download_button("⬇ 下載圖片", data=results[0],
                                file_name=f"fashion_{uuid.uuid4().hex[:6]}.png", mime="image/png")
                            st.info("💡 切換到「✏️ 繼續修改目前圖片」即可對這張圖繼續下指令修改！")
                        else:
                            # 多張：grid 顯示，最後一張設為 current
                            gcols = st.columns(3)
                            for idx, ib in enumerate(results):
                                with gcols[idx % 3]:
                                    ...
                                    st.download_button(f"⬇ 下載第 {idx+1} 張", data=ib,
                                        ...
                                        mime="image/png", key=f"dl_new_{idx}")
                                    if st.button(f"✏️ 以第 {idx+1} 張繼續修改", key=f"sel_{idx}"):
                                        st.session_state["current_img"]   = ib
                                        st.session_state["current_label"] = f"第{idx+1}張"
                                        st.session_state["edit_history"]  = [{
                                            "bytes":  ib,
                                            "prompt": full_prompt,
                                            "label":  f"原始生成 第{idx+1}張"
                                        }]
                                        st.rerun()
        ...
        with tab_edit:
            ...
            with ecol1:
                ...
                st.download_button("⬇ 下載目前版本", data=current_bytes,
                    file_name=f"fashion_v{version}_{uuid.uuid4().hex[:6]}.png", mime="image/png",
                    key="dl_current")

                # 清空 / 重新開始
                if st.button("🗑 清空，重新生成", key="clear_edit"):
                    st.session_state["current_img"]  = None
                    st.session_state["edit_history"] = []
            ...
            with ecol2:
                ...
                qcols = st.columns(4)
                for qi, (label, cmd) in enumerate(quick_cmds):
                    with qcols[qi % 4]:
                        if st.button(label, key=f"qcmd_{qi}", use_container_width=True):
                            st.session_state["quick_cmd"] = cmd

                edit_prompt = st.text_area(
                    "輸入修改指令",
                    value=st.session_state.pop("quick_cmd", ""),
                    placeholder="例如：把裙子改成紅色、加上蕾絲裝飾、換成純白背景...",
                    height=100,
                    key="edit_prompt_input"
                )

                # 畫質設定
                eq_key = st.selectbox("畫質／模型", list(QUALITY_MODES.keys()),
                    key="qk_edit", help="修改時使用的模型")
                er_key = st.selectbox("長寬比", list(ASPECT_RATIOS.keys()),
                    key="rk_edit")
                erez_key = st.selectbox("解析度", list(RESOLUTION_TIERS.keys()),
                    index=1, key="rek_edit")

                st.markdown(f'<p class="usage-counter">今日剩餘：{remaining} 張</p>', unsafe_allow_html=True)

                edit_btn = st.button("✏️ 套用修改", use_container_width=True, key="edit_apply")
                ...
                if edit_btn:
                    if not edit_prompt.strip():
                        ...
                        with st.spinner(f"AI 修改中（第 {version+1} 版），請稍候..."):
                            try:
                                new_bytes = generate_image_from_reference(
                                    current_bytes,
                                    build_prompt(edit_prompt),
