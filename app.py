...
IMAGE_SIZES = {
    "⬜ 正方形 1024×1024（預設）": {
        "size": "1024x1024",
        "desc": "通用商品圖、社群貼文",
        "compatible": ["gpt-image-2", "gpt-image-1", "dall-e-3", "dall-e-2"],
    },
    "🖼 橫式 1792×1024（寬螢幕）": {
        "size": "1792x1024",
        "desc": "廣告橫幅、官網首圖",
        "compatible": ["gpt-image-2", "gpt-image-1", "dall-e-3"],
    },
    "📱 直式 1024×1792（手機）": {
        "size": "1024x1792",
...
def generate_image_from_prompt(full_prompt, quality_key="🌟 Ultra (gpt-image-2)", size="1024x1024"):
...
def generate_image_from_reference(reference_bytes, full_prompt, quality_key="🌟 Ultra (gpt-image-2)", size="1024x1024"):
    ...
    for model in edit_models:
        try:
            ...
            kwargs = dict(model=model, image=png_file, prompt=full_prompt, size=size)
...
def show_generate(email, usage, daily_limit):
    ...
    with col2:
        ...
        size_str = sinfo.get("size", "1024x1024")
        ...
        if generate_btn:
            if not user_prompt.strip():
                ...
                selected_size = IMAGE_SIZES[size_key]["size"]
                with st.spinner("AI 生成中，請稍候..."):
                    try:
                        if reference_file:
                            ...
                            img_bytes = generate_image_from_prompt(full_prompt, quality_key, selected_size)
