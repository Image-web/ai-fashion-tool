...
def show_generate(email, usage, daily_limit):
    ...
    with st.container():
        ...
        with col1:
            ...
            # ── 多圖上傳 ──
            st.markdown('<p style="font-size:0.75rem;color:#888;letter-spacing:0.08em;text-transform:uppercase;margin-bottom:0.3rem;">上傳圖片（可多選，最多16張）</p>', unsafe_allow_html=True)
            ...
            # 預覽上傳的圖片
            if reference_files:
                n_files = len(reference_files)
                if n_files == 1:
                    mode_hint = "單圖修改模式"
                    mode_color = "#4A90D9"
                else:
                    mode_hint = f"多圖合成模式（{n_files} 張）— gpt-image-2 將綜合所有圖片生成"
                    mode_color = "#7B68EE"

                st.markdown(
                    f'<div style="background:#f0f0ff;border-left:3px solid {mode_color};'
                    f'padding:0.5rem 0.8rem;margin-bottom:0.5rem;font-size:0.8rem;color:#333;">'
                    f'📎 {mode_hint}</div>',
                    unsafe_allow_html=True
                )
                prev_cols = st.columns(min(n_files, 5))
                for pi, rf in enumerate(reference_files[:5]):
                    with prev_cols[pi]:
                        st.image(rf.getvalue(), use_container_width=True,
                                 caption=f"圖{pi+1} {rf.name[:10]}")
                if n_files > 5:
                    st.caption(f"...還有 {n_files-5} 張")

            # 多圖時顯示描述提示
            if reference_files and len(reference_files) > 1:
                st.markdown(
                    '<div style="background:#fff8e1;border-left:3px solid #FFC107;'
                    'padding:0.5rem 0.8rem;font-size:0.78rem;color:#555;">'
                    '💡 多圖合成提示：可在描述詞指定各圖的角色，例如：<br>'
                    '「將圖一的服裝款式保留，換上圖二模特的臉」<br>'
                    '「結合圖一的顏色配置和圖二的剪裁設計」</div>',
                    unsafe_allow_html=True
                )
        ...
        with col2:
            ...
            if n_refs == 1:
                cost_count = 1
                st.info("📌 單圖修改模式，消耗 1 張額度")
            elif n_refs > 1:
                cost_count = 1
                st.info(f"📌 多圖合成模式，{n_refs} 張輸入 → 1 張輸出，消耗 1 張額度")
            else:
        ...
        if generate_btn:
            if not user_prompt.strip():
                ...
                if reference_files:
                    n_ref = len(reference_files)

                    if n_ref == 1:
                        # ── 模式A：單圖修改 ──
                        with st.spinner(f"AI 修改中，請稍候..."):
                            try:
                                img_bytes = generate_image_from_reference(
                                    reference_files[0].getvalue(),
                                    full_prompt, quality_key, ratio_key, res_key)
                                results.append((img_bytes, reference_files[0].name))
                        ...
                        # ── 模式B：多圖合成（gpt-image-2 一次傳所有圖）──
                        st.info(f"📎 多圖合成中：將 {n_ref} 張圖片合併處理...")
                        with st.spinner("gpt-image-2 多圖合成中，請稍候（約 30-60 秒）..."):
                            try:
                                main_img   = reference_files[0].getvalue()
                                extra_imgs = [rf.getvalue() for rf in reference_files[1:]]
                                img_bytes  = generate_image_from_reference(
                                    main_img, full_prompt, quality_key, ratio_key, res_key,
                                    extra_images=extra_imgs)
                                label = f"合成結果（{n_ref}張輸入）"
                                results.append((img_bytes, label))
                                url = upload_to_supabase(img_bytes)
                                save_design(email, full_prompt, "", url)
                            except Exception as e:
                                st.error(f"多圖合成失敗：{e}")
